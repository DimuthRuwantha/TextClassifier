...
class NaiveBayes:
    ...
    """
    # Constructor
    def __init__(self, class_labels, token_dictionary):
        """
        ...
        token_dictionary: dictionary (a dictionary of terms and number of occurences of term in each class)
        """
        self.k = len(class_labels)
        self.priors = np.zeros((self.k, 1))             # prior probabilities for each class
        self.cctermp = np.zeros((len(token_dictionary), self.k))   # class conditional term probabilities
        self.ctermcnt = np.zeros((self.k, 1))           # total number of terms in a class
        self.lbl_dict = dict(zip(class_labels, range(self.k)))
        self.class_labels = class_labels
        self.tdict = token_dictionary
...
def main():
    ...
    id = 1
    start = dt.now()
    tokens = tokenizeDoc(test[class_titles[id]][3])     # store all the words/tokens
    lbl = dumbBayes.predict(tokens)
    end = dt.now()
